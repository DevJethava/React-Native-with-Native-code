...
public class NetInfo {
    ...
    @Override
    public int hashCode() {
        int ip_custom = prefs.getBoolean(Constant.KEY_IP_CUSTOM, Constant.DEFAULT_IP_CUSTOM) ? 1 : 0;
        int ip_start = prefs.getString(Constant.KEY_IP_START, Constant.DEFAULT_IP_START).hashCode();
        int ip_end = prefs.getString(Constant.KEY_IP_END, Constant.DEFAULT_IP_END).hashCode();
        int cidr_custom = prefs.getBoolean(Constant.KEY_CIDR_CUSTOM, Constant.DEFAULT_CIDR_CUSTOM) ? 1 : 0;
        int cidr = prefs.getString(Constant.KEY_CIDR, Constant.DEFAULT_CIDR).hashCode();
        return 42 + intf.hashCode() + ip.hashCode() + cidr + ip_custom + ip_start + ip_end + cidr_custom + cidr;
    }

    public void getIp() {
        intf = prefs.getString(Constant.KEY_INTF, Constant.DEFAULT_INTF);
        try {
            if (intf == Constant.DEFAULT_INTF || NOIF.equals(intf)) {
